...
@dataclass
class SentenceUnit:
    """Kleinstes semantisches Element im Chunking: ein Satz + Seitenindex."""
    page_idx: int
    text: str
...
def chunk_pages(
    ...
    Strategie:
    - Alle Seiten werden in Sätze zerlegt (SentenceUnits mit Seitenindex).
    ...
    Diese Strategie ist robuster als eine reine Seiten-/Zeichen-Heuristik und erzeugt
    Chunks, die für sich sinnvoll lesbar sind (wichtig für Embeddings, RAG & Q/A).
    """
    # 1) Seiten in Sätze zerlegen und lineare Sequenz von SentenceUnits aufbauen.
    units: List[SentenceUnit] = []
    for page_idx, page_text in enumerate(page_texts):
        if not page_text:
            # Leere Seite überspringen, Seitenindex aber in meta belassen
            continue
        sentences = _split_into_sentences(page_text)
        if not sentences:
            continue
        for s in sentences:
            units.append(SentenceUnit(page_idx=page_idx, text=s))
    ...
    def _current_len(sentences: List[SentenceUnit]) -> int:
        ...
        if not sentences:
            return 0
        # Ein Leerzeichen zwischen Sätzen als Separator
    ...
    for unit in units:
        ...
        if sent_len >= max_chars:
            if current:
                ...
                current = []
            chunks.append(Chunk(text=sent_text, page_start=unit.page_idx, page_end=unit.page_idx))
            continue
        ...
        if _current_len(tentative) <= max_chars:
            ...
            if sentence_overlap > 0:
                # Tail (Überlappung) merken, bevor wir finalisieren
            ...
            # Falls der Overlap alleine schon zu groß ist, den Overlap verwerfen
            if _current_len(current) > max_chars:
                current = []

            # Jetzt den neuen Satz anhängen (ggf. als alleinigen Chunk)
            if not current:
                # Kein Overlap oder Overlap verworfen
                current = [unit]
            else:
                tentative = current + [unit]
                if _current_len(tentative) <= max_chars:
                    ...
                    # Overlap + Satz wäre zu groß → Overlap als Chunk war schon geschrieben,
