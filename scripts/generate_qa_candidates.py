...
def load_or_default_prompts(cfg: QAConfig) -> Tuple[str, str]:
    ...
    if system_prompt is None:
        system_prompt = (
            ...
            "- If the chunks do not explicitly support a fully grounded, self-contained Q/A, return an empty JSON array: [].\n"
            ...
            "- Do NOT mention chunk_id, doc_id, file paths, FAISS, or any metadata.\n\n"
            "Grounding check (must pass):\n"
            "- Every sentence in the answer must be directly supported by an explicit statement in the chunks.\n"
    ...
    if user_template is None:
        user_template = (
            ...
            "2) Relationships between quantities explicitly stated (including sign/direction of effects if stated).\n"
            "3) Meaning/interpretation of symbols/terms explicitly defined in the chunks.\n"
            "4) Comparisons ONLY if the chunks explicitly compare (do NOT ask for 'advantages/limitations' unless listed).\n\n"
            "Avoid:\n"
            "- Vague, generic engineering knowledge.\n"
            "- Questions that require filling gaps or adding missing reasoning.\n"
            "- Near-duplicate questions.\n\n"
            "Length constraints:\n"
            "- Question: max 2 sentences.\n"
            "- Answer: max 4 sentences (unless the chunks explicitly contain a longer step list).\n"
            "- If the chunks only support a short factual answer, keep it short.\n\n"
            "Difficulty label:\n"
