...
def load_or_default_prompts(cfg: QAConfig) -> Tuple[str, str]:
    ...
    if system_prompt is None:
        system_prompt = (
            ...
            "Generate exam-style and practice question–answer pairs STRICTLY from the provided context chunks.\n\n"
            "Non-negotiable rules:\n"
            "- Use ONLY information explicitly stated in the chunks. Do NOT add outside knowledge.\n"
            "- Do NOT invent formulas, symbols, parameter values, assumptions, units, definitions, or steps.\n"
            "- If a grounded, self-contained Q/A cannot be produced from the chunks, return an empty JSON array: [].\n"
            "- Keep wording, symbols, and notation exactly as in the chunks (Greek letters, subscripts, units, etc.).\n"
            "- Use the same main language as the context (German vs. English). If mixed/unclear, use the language that dominates.\n"
            "- Be precise and technically correct. Prefer compact answers.\n"
    ...
    if user_template is None:
        user_template = (
            ...
            "You must NOT mention chunk_id/doc_id in the question or answer.\n\n"
            "{CONTEXT}\n\n"
            "Task:\n"
            "- Generate BETWEEN 1 and {MAX_QA_PER_GROUP} high-quality, non-trivial question–answer pairs.\n"
            "- Each pair must be fully grounded in the chunks. If you cannot meet this, return [].\n\n"
            "Quality targets (use these as a checklist):\n"
            "1) Non-trivial: requires understanding (assumptions, validity, relations, meaning of equations), not mere copying.\n"
            "2) Self-contained: a student can answer using the provided information, without seeing chunk labels.\n"
            "3) Faithful: no added facts, no guessed missing steps.\n"
            "4) Diverse: avoid near-duplicate questions; cover different aspects if multiple pairs are generated.\n\n"
            "When formulas/derivations/numerical relations appear:\n"
            "- Ask for interpretation, conditions of validity, qualitative effects, or meaning of terms.\n"
            "- Do NOT introduce new symbols or derivation steps that are not explicitly present.\n\n"
            "Length constraints:\n"
            "- Question: max 2 sentences.\n"
            "- Answer: max 4 sentences (unless the chunks explicitly contain a longer step list).\n\n"
            "Difficulty label:\n"
            "- Set \"difficulty\" to one of: \"basic\", \"intermediate\", \"advanced\".\n"
            "- basic: direct recall or single-step application explicitly shown in the chunks.\n"
            "- intermediate: combines 2+ facts/conditions from the chunks.\n"
            ...
            "If you cannot produce at least one grounded pair, output exactly:\n"
            "[]\n"
        )
